...
voxels = np.vstack([np.tile(network, (extent, 1)) for network, extent
                    in zip(networks, seven)])
...
structured_noise = np.vstack([np.tile(
    (noise_sources[np.random.choice(range(7)), :] *
     np.random.uniform(.6, 1.0) * np.random.choice([-1, 1, 1, 1])),
    (extent, 1))
                              for extent in areas])
...
data = np.vstack([np.tile(pattern, (extent, 1))
                  for pattern, extent in zip(patterns, twelve)])
